...
function Navbar() {
    ...
    const { setScope, solvedIds } = useContext(QuestionContext)
            ...
            {profile &&
                <div className="w-[500px] h-[400px] bg-black fixed top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 border-2 border-green-500 flex flex-col text-white">
                    ...
                    <div className="flex mt-5 ml-5">
                        <div>Name</div><div className="ml-28 p-2 border-none outline-none bg-white text-black rounded w-[250px]">{user.name}</div>
                    </div>
                    <div className="flex mt-5 ml-5">
                        <div>Email</div><div className="ml-28 p-2 border-none outline-none bg-white text-black rounded w-[250px]">{user.email}</div>
                    </div>
                    <div className="flex mt-5 ml-5">
                        <div>Problems Solved</div><div className="ml-8 p-2 border-none outline-none bg-white text-black rounded w-[250px]">{solvedIds.length}</div>
                    </div>
